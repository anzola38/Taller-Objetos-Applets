...
import MetodosDeOrdenamiento.BubbleSort;
import MetodosDeOrdenamiento.CountingSort;
import MetodosDeOrdenamiento.MergeSort;
import MetodosDeOrdenamiento.MergeSort2;
import MetodosDeOrdenamiento.QuickSort;
import MetodosDeOrdenamiento.RadixSort;
...
public class Applet extends JApplet {
	...
	public void Ordenar(int[] numeros, int index, String[] numeros2) {
		...
		if (index == 0) {
			...
			panelusuario.imprimir(bubblesort.burbuja(numeros));

		} else {

			if (index == 1) {

				panelusuario.imprimir2(radixsort.sort(numeros2));

			} else {

				if (index == 2) {

					panelusuario.imprimir(MergeSort2.sort(numeros));

				} else {

					if (index == 3) {

						panelusuario.imprimir(quicksort.sort(numeros));

					} else {

						if (index == 4) {

							panelusuario.imprimir(countingsort.countingSort(numeros));
	...
	public String guardar(String informacion) {

		Archivo archivo = new Archivo();

		return archivo.guardarArchivo(informacion);
